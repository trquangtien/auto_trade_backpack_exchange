...
const sellfun = async client => {
  ...
  if (GetOpenOrders.length > 0) {
    let CancelOpenOrders = await client.CancelOpenOrders({
      symbol: 'SOL_USDC'
    });
  ...
  let finalLastPriceAsk = lastPriceask - 1;
  ...
  console.log(
    getNowFormatDate(),
    `Trade... ${quantitys} at ${lastPriceask}$ $SOL to ${(lastPriceask * quantitys).toFixed(
      2
    )} $USDC`
  );
  let orderResultAsk = await client.ExecuteOrder({
    orderType: 'Limit',
    price: finalLastPriceAsk.toString(),
    quantity: quantitys,
    side: 'Ask',
    symbol: 'SOL_USDC',
    timeInForce: 'IOC'
...
const buyfun = async client => {
  ...
  console.log(
    getNowFormatDate(),
    `My Account Infos: ${userbalance.SOL.available} $SOL | ${userbalance.USDC.available} $USDC`
  );
